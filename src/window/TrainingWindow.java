...
public class TrainingWindow extends JFrame implements ItemListener, ActionListener{
	...
	JCheckBox continuous, presetCheck;
	...
	public void addCoinSelect() {
		...
		JLabel instr2 = new JLabel("Train All");
		instr2.setBounds(locX + comboW + 20, distanceY * offset, 100, comboH);
		
		continuous = new JCheckBox();
		continuous.addActionListener(this);
		continuous.setFocusable(false);
		continuous.setBounds(locX + comboW + 70, distanceY * offset, comboH, comboH);
	...
	public void addPreset() {
		...
		presetCheck.setBounds(locX + comboW + 70, 0, comboH, comboH);
		
		JLabel instr2 = new JLabel("Preset");
		instr2.setBounds(locX + comboW + 20, 0, 100, comboH);
		
		
		presetPanel.add(preset);
		presetPanel.add(instr);
		
		presetPanel.add(presetCheck);
		presetPanel.add(instr2);
	...
	public void addIterations() {
		...
		JLabel prompt = new JLabel("Enter Iterations");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == finish) {
			...
			if(continuous.isSelected()) {
				...
				train = new TrainingOrder(dataset, gran, null, null, iterations, true, presetCheck.isSelected());
				
			} else {
				if(!presetCheck.isSelected()) presetName = null;
				train = new TrainingOrder(dataset, gran, coin, presetName, iterations, false, false);
			}
		...
		if(e.getSource() == continuous) {
			//if(continuous.isSelected()) presetCheck.setSelected(false);
		}
		
		if(e.getSource() == presetCheck) {
			if(!presetCheck.isSelected()) presetName = null;
